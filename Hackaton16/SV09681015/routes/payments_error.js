...
router.post('/refund', isAuthenticatedAPI, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const [[refundRow]] = await conn.query(
        'CALL sp_process_refund(?, ?, ?, @refund_id, @amount); SELECT @refund_id AS refund_id, @amount AS amount;',
        [orderId, req.user.id, reason]
      );

      // Obtener el stripe_payment_id de la orden
